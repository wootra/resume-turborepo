	...
	if (ref.current) {
		...
		if (size.w > size.h) {
			return {
				...
				yScale: d3.scaleLinear([0, (100 * size.h) / size.w], [MARGIN, size.h]),
				w: 100,
				h: (100 * size.h) / size.w,
			};
		} else {
			return {
				xScale: d3.scaleLinear([0, (100 * size.w) / size.h], [MARGIN, size.w]),
...
const getNextBallInfo = (prevBallInfo: BallInfo, timeDiff: number, acc: { x: number; y: number }, scales: Scales) => {
	...
	const xSDir = xSpeed > 0 ? 1 : -1;
	const ySDir = ySpeed > 0 ? 1 : -1;
	...
	const ballInfo = {
		...
		xSpeed: (Math.min(Math.abs(xSpeed), 40) * xSDir + acc.x * timeDiff * SPEED_RESOLUTION) * xDir,
		ySpeed: (Math.min(Math.abs(ySpeed), 40) * ySDir + acc.y * timeDiff * SPEED_RESOLUTION) * yDir,
	};
	return ballInfo;
};
...
const GamificBackgroundPulse = () => {
	...
	const pulseCb = useCallback((frame: number) => {
		if (animationRef.current && containerRef.current && scalesRef.current) {
			...
			if (timeDiff > FRAME_LEN) {
				ballInfoRef.current = getNextBallInfo(ballInfoRef.current, timeDiff, acc.current, scalesRef.current);
				if (ballRef.current && isAnimnationOnRef.current) {
					const { xScale, yScale } = scalesRef.current;
					ballRef.current.setAttribute(
						'style',
						`left: ${xScale(ballInfoRef.current.x)}px; top: ${yScale(ballInfoRef.current.y)}px`
					);
				}
	...
	return (
		<>
			...
			<div className='absolute inset-0 pointer-events-none border-2 border-green' ref={containerRef}>
				<div
					ref={ballRef}
