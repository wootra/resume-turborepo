...
const GamificBackground = () => {
			...
			if (ref.current) {
				...
				const newPoints: Pos[] = points.map(pos => {
					...
					const nSize = Math.min(
						1,
						Math.sqrt(Math.pow(Math.abs(x - sx), 2) + Math.pow(Math.abs(y - sy), 2)) / OPEN_RADIUS
					);

					if (nSize !== size) {
						return {
							...pos,
							color: nSize === 1 ? '#ffffff' : `hsl(${colorBase}, ${100 * size}%, ${100 * size}%)`,
							size: nSize,
						};
					}
	...
	return (
		...
		>
			<svg className='w-full rounded-full' viewBox={`0 0 ${NUM_OF_BOXES} ${NUM_OF_BOXES}`}>
				{points.map(d => {
					...
					return (
						<g key={d.key} transform={`translate(${x + 1 / 2},${y + 1 / 2})`}>
							<rect
								x={-0.5}
								y={-0.5}
								...
								style={
									size === 1
										? {}
										: {
												transform: `rotate(${(-90 * d.size).toFixed(2)}deg)`,
										  }
								}
								width={size * 1.1}
								height={size * 1.1}
