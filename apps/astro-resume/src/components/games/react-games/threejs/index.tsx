...
// import GreyButton from '../../../../../src/components/app-comps/react-libraries/components/GreyButton';
import WebGL from 'three/addons/capabilities/WebGL.js';
import GreyButton from '@/components/app-comps/react-libraries/components/GreyButton';
		...
		<div className='flex flex-col gap-4 min-w-[calc(100vw-8rem)] min-h-[calc(100vh-8rem)]'>
			<div className='flex flex-row gap-2'>
				...
				{examples.map(({ compoType, label }) => (
					<GreyButton key={compoType} onClick={() => setCompo(compoType)} selected={compo === compoType}>
						{label}
					</GreyButton>
				))}
			...
			<div className='w-full h-full flex-1 relative border border-gray' ref={ref}>
				<div className='absolute inset-0'>
					{examples.map(
						({ compoType, Component }) =>
							compo === compoType &&
							WebGL.isWebGL2Available() && (
								<Suspense key={compoType} fallback={<AutoLoading loadingTime={1000} />}>
					...
					{compo === 'none' && WebGL.isWebGL2Available() && (
						<div className='grid place-items-center h-full'>Please select one</div>
					)}
					{!WebGL.isWebGL2Available() && (
						<div className='grid place-items-center h-full'>WebGL is not available in this browser...</div>
					)}
				</div>
			</div>
		</div>
